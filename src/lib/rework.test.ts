...
// The extracted steps, imported directly: each is unit-tested below on its own, beside the
// end-to-end cases that drive them through `reworkTicket`.
const { validateReworkInput } = await import("./rework-contract");
const { followUpDescription, hasAnyHumanNote, hasHumanNote, originNoteBody, settledPhrase } =
  await import("./rework-notes");
...
/**
 * The steps `reworkTicket` is composed of, each driven directly (anton-9rtj). The cases above prove
 * the whole send-back; these pin the contract of each piece, so a later change to one is caught where
 * it happens rather than as a puzzling end-to-end failure.
 */
describe("validateReworkInput (rework-contract)", () => {
  it("returns the request trimmed, with findings defaulted", () => {
    expect(
      validateReworkInput({
        ticketId: " t1 ",
        mode: "follow-up",
  ...
  it("names the field that is missing, and the field that is too long", () => {
    ...
    expect(() => validateReworkInput(input({ summary: "x".repeat(201) }))).toThrow(
      /Summary is too long \(201 > 200/,
    );
    expect(() => validateReworkInput(input({ instructions: "x".repeat(2001) }))).toThrow(
      /Instructions are too long \(2001 > 2000/,
    );
  });
  ...
  it("refuses a mode it doesn't implement rather than guessing one", () => {
    ...
    expect(() => validateReworkInput(input())).not.toThrow();
  });
});

describe("rework-notes", () => {
  const noteArgs = {
  ...
  it("reworkNoteBody says which judgement the founder made, per mode", () => {
    expect(reworkNoteBody({ ...noteArgs, mode: "reopen" })).toContain(
      "Rework — acceptance not met. Sent back from feat's self-review",
    );
    expect(reworkNoteBody(noteArgs)).toContain("Follow-up on t1 — its acceptance stands");
  });

  it("reworkNoteBody renders a REDIRECTED send-back apart — it says the opposite thing", () => {
  ...
  it("followUpDescription writes every section the bead contract judges", () => {
    ...
    for (const heading of ["## Goal", "## Context", "## Out of scope", "## Verify"]) {
      expect(description).toContain(heading);
    }
    expect(description).toContain("It runs as a ticket of feat");
    // The instructions live in the human note, never duplicated into the bead body.
    expect(description).not.toContain(INSTRUCTIONS);
  });

  it("followUpDescription tells a parentless bead it is its own run target", () => {
    const description = followUpDescription({
      summary: "harden the retry",
      ticket: ticketA(),
      targetId: "feat",
    });
    expect(description).toContain("It is its own run target — approve it to run.");
  });

  it("followUpDescription's provenance says WHY the fix moved, when a merge redirected it", () => {
    const description = followUpDescription({
      summary: "harden the retry",
      ticket: ticketA(),
      targetId: "feat",
    ...
    expect(description).toContain("had already merged, so this bead carries the fix");
  });

  it("originNoteBody keeps a redirected pointer from claiming the acceptance stood", () => {
    expect(originNoteBody("anton-new")).toContain("its acceptance stands");
    const redirected = originNoteBody("anton-new", {
      outcome: "shipped",
      pr: "gh-42",
      redirected: true,
    });
    expect(redirected).toContain("judged its acceptance unmet");
    expect(redirected).not.toContain("its acceptance stands");
  });

  it("settledPhrase keeps 'unreadable' distinct from a state change — they are fixed differently", () => {
    expect(settledPhrase("gh-42", "merged")).toBe("gh-42 reads as merged now");
    expect(settledPhrase("gh-42", "unknown")).toBe(
      "gh-42's state could no longer be read as it was applying",
    );
  });

  it("hasHumanNote matches its own blob back through the note header and whitespace", () => {
    const body = reworkNoteBody(noteArgs);
    const bead = makeBead({ id: "t1", notes: formatHumanNote(body, "founder", new Date()) });
    expect(hasHumanNote(bead, body)).toBe(true);
    expect(hasHumanNote(bead, `${body}\n\nand one more thing`)).toBe(false);
    expect(hasHumanNote(makeBead({ id: "t1" }), body)).toBe(false);
  });

  it("hasAnyHumanNote ignores anton's own bookkeeping — only a request leaves a human note", () => {
...
describe("rework-target", () => {
  it("runMembers is the feature's tickets, or a standalone target itself", () => {
    const all = [feature(), ticketA(), ticketB()];
    expect(runMembers(all[0], all).map((b) => b.id)).toEqual(["t1", "t2"]);
    const solo = makeBead({ id: "solo", issue_type: "task" });
    expect(runMembers(solo, [solo]).map((b) => b.id)).toEqual(["solo"]);
  });

  it("assertNoLiveRun refuses this machine's run and another machine's lease alike", () => {
    runIsLiveMock.mockReturnValue(true);
    expect(() => assertNoLiveRun("p1", feature())).toThrow(ReworkConflictError);

    runIsLiveMock.mockReturnValue(false);
    const leased = makeBead({
      id: "feat",
      labels: [`run-lease:${Date.now() + 60_000}:other-run`],
    });
    expect(() => assertNoLiveRun("p1", leased)).toThrow(ReworkConflictError);
    // A lapsed lease is a stopped run, not a live one.
    const lapsed = makeBead({ id: "feat", labels: [`run-lease:${Date.now() - 60_000}:dead-run`] });
    expect(() => assertNoLiveRun("p1", lapsed)).not.toThrow();
  });

  it("resolveReworkScope returns the fresh board with the pair it vetted", async () => {
  ...
  it("resolveReworkScope separates 'no such bead' from 'not a run target' from 'not its ticket'", async () => {
...
describe("rework-pipeline", () => {
  /** A target carrying a live PR ref — what `resolvePipeline` and the retire both read first. */
  const withPr = (pr = "gh-42") => makeBead({ id: "feat", issue_type: "feature", metadata: { pr } });

  it("stageLabelsOn reports only the run stage labels the bead is actually wearing", () => {
  ...
  it("resolvePipeline says nothing about a target with no PR at all — and never asks gh", async () => {
    await expect(resolvePipeline("/repo", feature(), "reopen")).resolves.toBeUndefined();
    expect(prStateMock).not.toHaveBeenCalled();
  });

  it("resolvePipeline retires an OPEN live ref and ships a MERGED one, redirecting the reopen", async () => {
    prStateMock.mockResolvedValue("open");
  ...
  it("resolvePipeline reads an already-RETIRED ref for the merge it may have missed", async () => {
    const retired = makeBead({ id: "feat", issue_type: "feature", metadata: { retiredPr: "gh-42" } });
    prStateMock.mockResolvedValue("merged");
  ...
  it("resolvePipeline refuses an unreadable PR rather than picking one of two opposite answers", async () => {
    prStateMock.mockResolvedValue("unknown");
  ...
  it("planRework spares gh entirely for a follow-up that comes out parentless anyway", async () => {
    const solo = makeBead({ id: "solo", issue_type: "task", metadata: { pr: "gh-42" } });
    const plan = await planRework("/repo", solo, solo, [solo], "follow-up");
    expect(prStateMock).not.toHaveBeenCalled();
    expect(plan).toEqual({ pipeline: undefined, mode: "follow-up", rollsBackTicket: false });
  });

  it("planRework carries the PR's redirect into the mode it plans", async () => {
    prStateMock.mockResolvedValue("merged");
    const plan = await planRework("/repo", withPr(), ticketA(), [withPr(), ticketA()], "reopen");
  ...
  it("planRework flags a child rollback only for a reopen of a bead that is not the target", async () => {
    const all = [feature(), ticketA()];
    await expect(planRework("/repo", feature(), ticketA(), all, "reopen")).resolves.toMatchObject({
  ...
  it("snapshotBeforeWrites reads the ticket too, but only where a rollback would reach it", async () => {
    showMock.mockImplementation(async (_cwd, id) => makeBead({ id, status: "closed" }));
    const both = await snapshotBeforeWrites("/repo", feature(), ticketA(), true);
    expect([both.target.id, both.ticket?.id]).toEqual(["feat", "t1"]);
    expect(await snapshotBeforeWrites("/repo", feature(), ticketA(), false)).toMatchObject({
      ticket: undefined,
    });
  });

  it("snapshotBeforeWrites refuses before any write when the PR stopped being open", async () => {
    showMock.mockImplementation(async (_cwd, id) => makeBead({ id, metadata: { pr: "gh-42" } }));
    prStateMock.mockResolvedValue("merged");
    await expect(snapshotBeforeWrites("/repo", feature(), ticketA(), false)).rejects.toBeInstanceOf(
      ReworkConflictError,
    );
    prStateMock.mockResolvedValue("unknown");
    await expect(snapshotBeforeWrites("/repo", feature(), ticketA(), false)).rejects.toBeInstanceOf(
  ...
  it("retireFinishedRun moves the live ref LAST, so a failure part-way is re-enterable", async () => {
    showMock.mockImplementation(async (_cwd, id) =>
      makeBead({ id, status: "closed", metadata: { pr: "gh-42" } }),
    );
    prStateMock.mockResolvedValue("open");
    const before = makeBead({ id: "feat", status: "closed", labels: ["stage:in-review"] });

    await expect(retireFinishedRun(project, before)).resolves.toEqual({ outcome: "retired" });

    expect(untagMock).toHaveBeenCalledWith("/repo", "feat", ["stage:implementing", "stage:in-review"]);
    expect(reopenMock).toHaveBeenCalledWith("/repo", "feat", "rework: sent back for another round");
    expect(retirePrRefMock.mock.invocationCallOrder[0]).toBeGreaterThan(
      untagMock.mock.invocationCallOrder[0],
    );
    expect(noteMock.mock.calls[0][2]).toContain("gh-42 is still open");
  });

  it("retireFinishedRun is a no-op on a target whose ref is already gone", async () => {
    showMock.mockImplementation(async (_cwd, id) => makeBead({ id }));
    await expect(retireFinishedRun(project, makeBead({ id: "feat" }))).resolves.toEqual({
      outcome: "already-retired",
    });
    expect(untagMock).not.toHaveBeenCalled();
  });

  it("retireFinishedRun undoes its own writes when the verify no longer reads the PR as open", async () => {
    showMock.mockImplementation(async (_cwd, id) =>
      makeBead({ id, status: "closed", metadata: { pr: "gh-42" } }),
    );
    prStateMock.mockResolvedValue("merged");
    const before = makeBead({ id: "feat", status: "closed", labels: ["stage:in-review"] });
    const ticketBefore = makeBead({ id: "t1", status: "closed", labels: ["stage:in-review"] });

    await expect(retireFinishedRun(project, before, ticketBefore)).resolves.toEqual({
      outcome: "raced",
      pr: "gh-42",
      state: "merged",
    });

    // Target and child both put back, and the live ref restored last of the target's three writes.
  ...
  it("retireFinishedRun reports an unreadable verify apart from a lost race", async () => {
    showMock.mockImplementation(async (_cwd, id) => makeBead({ id, metadata: { pr: "gh-42" } }));
    prStateMock.mockResolvedValue("unknown");
    await expect(retireFinishedRun(project, makeBead({ id: "feat" }))).resolves.toEqual({
  ...
  it("retireWrote counts every outcome that touched the board, including a rolled-back one", () => {
  ...
  it("assertRetireStood is silent for a retire that stood, and for one never attempted", () => {
    const applied = {
      targetId: "feat",
      ticketId: "t1",
      reworkedId: "t1",
  ...
  it("assertRetireStood names both rolled-back beads on a grouped reopen", () => {
    const applied = { targetId: "feat", ticketId: "t1", reworkedId: "t1", rollsBackTicket: true };
    expect(() => assertRetireStood({ outcome: "raced", pr: "gh-42", state: "merged" }, applied))
      .toThrow(/applied to feat and t1 was rolled back/);
    expect(() =>
      assertRetireStood({ outcome: "raced", pr: "gh-42", state: "merged" }, {
        ...applied,
        rollsBackTicket: false,
      }),
    ).toThrow(/applied to feat was rolled back/);
  });

  it("assertRetireStood separates 'the PR moved' (409) from 'anton went blind' (503)", () => {
...
describe("rework-modes", () => {
  ...
  it("applyReopen writes the note, then the status, then strips the run's stage labels", async () => {
    const applied = await applyReopen(project, feature(), ticketA(), request);

    expect(applied).toMatchObject({ runsUnderTarget: true, result: { applied: true } });
    expect(noteMock.mock.invocationCallOrder[0]).toBeLessThan(reopenMock.mock.invocationCallOrder[0]);
    expect(untagMock).toHaveBeenCalledWith("/repo", "t1", ["stage:implementing", "stage:in-review"]);
  });

  it("applyReopen is a no-op only when the note AND the state it produces are already there", async () => {
    const body = reworkNoteBody({
      mode: "reopen",
      targetId: "feat",
    ...
    showsWithNote("t1", body, { status: "open", labels: [] });
    await expect(applyReopen(project, feature(), ticketA(), request)).resolves.toMatchObject({
      result: { applied: false },
    });
    expect(noteMock).not.toHaveBeenCalled();

    // Same note, but a later run re-closed the bead: on text alone this would leave it closed.
    showsWithNote("t1", body, { status: "closed" });
    await expect(applyReopen(project, feature(), ticketA(), request)).resolves.toMatchObject({
      result: { applied: true },
    });
  ...
  it("applyFollowUp parents under a board card, and reports that the target's run carries it", async () => {
    const applied = await applyFollowUp(project, feature(), ticketA(), {
      ...request,
      mode: "follow-up",
      summary: "Harden the retry path",
    });

    expect(applied).toMatchObject({ runsUnderTarget: true, result: { reworkedId: "anton-new" } });
    expect(createMock.mock.calls[0][1].deps).toEqual(["parent-child:feat"]);
    // The provenance edge is written before the instructions land on the new bead.
    expect(linkMock.mock.invocationCallOrder[0]).toBeLessThan(noteMock.mock.invocationCallOrder[0]);
  });

  it("applyFollowUp leaves a SHIPPED target's follow-up parentless — its run has nothing left", async () => {
    const applied = await applyFollowUp(
      project,
      feature(),
      ticketA(),
      { ...request, mode: "follow-up", summary: "Harden the retry path" },
      { outcome: "shipped", pr: "gh-42", redirected: false },
    );
    expect(applied.runsUnderTarget).toBe(false);
    expect(createMock.mock.calls[0][1].deps).toBeUndefined();
  });

  it("existingFollowUp prefers the bead carrying THIS request's note over an unfinished one", async () => {
    ...
    const all = [candidate("half"), candidate("done")];
    showsWithNote("done", body);

    await expect(existingFollowUp("/repo", all, "t1", "Harden the retry path", body)).resolves.toEqual({
  ...
  it("existingFollowUp adopts a bead with NO human note as this request's half-done work", async () => {
    const half = makeBead({
      id: "half",
      title: "Harden the retry path",
      dependencies: [{ issue_id: "half", depends_on_id: "t1", type: "discovered-from" }],
    });
    showMock.mockImplementation(async () => half);
    await expect(
      existingFollowUp("/repo", [half], "t1", "Harden the retry path", "a body"),
    ).resolves.toMatchObject({ partial: true, bead: { id: "half" } });
  });

  it("existingFollowUp ignores a bead that is closed, differently titled, or not discovered-from", async () => {
    const linked = (over: Partial<Bead> & { id: string }) =>
      makeBead({
        title: "Harden the retry path",
        dependencies: [{ issue_id: over.id, depends_on_id: "t1", type: "discovered-from" }],
        ...over,
      });
    const board: Bead[] = [
      linked({ id: "closed", status: "closed" }),
      linked({ id: "retitled", title: "Something else" }),
      makeBead({ id: "unlinked", title: "Harden the retry path" }),
    ];
    await expect(
      existingFollowUp("/repo", board, "t1", "Harden the retry path", "a body"),
    ).resolves.toBeUndefined();
    expect(showMock).not.toHaveBeenCalled();
  });

  it("inheritedLabels carries routing and shaping over, and nothing that describes a run", () => {
    const ticket = makeBead({
      id: "t1",
      labels: ["agent:nextjs", "domain:eng", "risk:low", "size:M", "area:codehealth", "approved", "stage:in-review", "review-score:4"],
    });
    expect(inheritedLabels(ticket)).toEqual([
      "agent:nextjs",
      "domain:eng",
      "risk:low",
      "size:M",
