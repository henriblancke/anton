 ...
 *
 * The approve route gates a run target on three questions before it labels anything `approved`: does
 * the bead contract hold over the set the run will dispatch (anton-6u6y / anton-j9zs), does the tier
 * structure hold under it, and is anything still blocking it. Approval is a MOMENT, though, and the
 * board keeps moving after it: an Acceptance section can be edited away, a feature can land under a
 * legacy epic, a `blocks` edge can be drawn — all while the target sits approved in the queue waiting
 * for a worker.
 *
 * So the same three questions are asked again on the product-master cadence (pm/revalidate.ts), and
 * a third time when the resulting proposal is approved (gardener/apply.ts). This module is what they
 * share. It does NOT re-implement any of them: `contractGaps` over `contractGatedBeads`,
 * `structureGaps`, and the epic-graph blocker rollup are the very functions the route calls, composed
 * here in one place so a re-check can never hold a target to a different bar than the gate did.
 *
 ...
 * and body (422 contract, 422 structure, 409 blocked) and needs the advisory half besides, which a
 * merged list cannot express.
 */
...
/** Which of approval's three promises a target has stopped keeping. */
export type ApprovalRule = "contract" | "structure" | "blocked";
