...
// ── bd exec seam (anton-n1m0) ──
//
// The bd spawn and the one sync pass it runs live in ./dolt-exec: it owns HOW anton talks to bd,
// while ./sync-coalescer owns WHEN a pass runs and imports that pass from there too — so neither
// module imports this one back and the two sides are testable apart. Re-exported here so every
// existing `from ".../beads/bd"` import keeps working.
export {
  BD_KILL_GRACE_ENV,
  BD_MAX_BUFFER_ENV,
  BD_STEP_TIMEOUT_ENV,
...
export type { BdExec, SyncMode, SyncOutcome } from "./dolt-exec";
...
// ── Sync state seam (anton-ladt) ──
//
// The coalescer and the per-repo sync-status registry live in ./sync-coalescer, which decides WHEN
// a pass runs; ./dolt-exec owns HOW one runs. Re-exported here so bd.ts stays the single import
// site for beads state.
