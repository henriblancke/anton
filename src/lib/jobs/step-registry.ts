 ...
 *
 * FAIL LOUD on anything else: a step carrying no `step:` label, or one naming a handler that doesn't
 * exist, PARKS the run with the step id, the label, and the formula file — never a silent skip,
 * which would let a formula define a run that quietly never opens a PR.
 */
...
async function loadStepReasoning(ctx: StepContext, stepId: string): Promise<string> {
  ...
  throw new PoisonEpic(
    `formula step "${stepId}" is a \`${STEP_LABEL_PREFIX}:claude\` step but names no prompt — add a ` +
      `\`prompt:<id>\` label (a \`.claude/agents/<id>.md\`) or a \`skill:<id>\` label (a ` +
      `\`.claude/skills/<id>/SKILL.md\`) so anton knows what to dispatch`,
  );
}
