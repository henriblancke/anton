 ...
 *
 * Kept out of `worktree-reaper.ts` so the job wiring there stays a thin dispatcher over this decision
 * + support layer — see anton-ikl3.
 */
...
/** {@link makeRevalidator}'s async body, named so the factory itself stays a one-liner. */
...
export function makeRevalidator(args: {
  ...
  return (candidate) => revalidateCandidate(args, candidate);
}

/** {@link runWorktreeReaperPass}'s deps, with every default applied once. */
interface ResolvedReaperDeps {
  db: AntonDb;
  clock: Clock;
  branchPrefix: string;
  readBoard: (repoPath: string) => Promise<Bead[]>;
...
function resolveReaperDeps(deps: WorktreeReaperDeps): ResolvedReaperDeps {
  return {
    db: deps.db,
    clock: deps.clock ?? systemClock,
    branchPrefix: deps.branchPrefix ?? "anton",
    readBoard: deps.readBoard ?? ((repo: string) => readBoardOrFail(repo)),
    lookupPr: deps.lookupPr,
    showBead: deps.showBead ?? beads.show,
  };
}

/** This project's run rows — every status, since a live row still names what it is using. */
async function fetchRunRows(db: AntonDb, projectId: string) {
  return db
    .select({
...
/** The residue candidates for this pass — both git reads are what BOUNDS the sweep to what still exists. */
async function buildCandidates(
  deps: ResolvedReaperDeps,
  repo: string,
  projectId: string,
  board: Bead[],
): Promise<ReapCandidate[]> {
  const runs = await fetchRunRows(deps.db, projectId);
  ...
  const [worktrees, branches] = await Promise.all([
    listWorktrees(repo),
    listBranches(repo, deps.branchPrefix),
  ]);
  return reapCandidates({
    repoPath: repo,
    worktrees,
    branches,
    runs,
...
/** Run the sweep over `candidates` and account for it — a session only when there is something to say. */
async function sweepAndReport(
  ctx: JobContext,
  deps: ResolvedReaperDeps,
  repo: string,
  projectId: string,
  ...
  await ctx.heartbeat();
  const session = deferPassSession(deps.db, deps.clock, { ctx, projectId, kind: "worktree-reaper" });
  const report = await reapWorktrees({
    repoPath: repo,
    candidates,
    lookupPr: deps.lookupPr,
    revalidate: makeRevalidator({ db: deps.db, projectId, repoPath: repo, showBead: deps.showBead }),
...
/** The worktree-reaper job's full pass, over one project. `makeWorktreeReaperHandler` just dispatches here. */
export async function runWorktreeReaperPass(ctx: JobContext, rawDeps: WorktreeReaperDeps): Promise<void> {
  const deps = resolveReaperDeps(rawDeps);
  ...
  const board = await deps.readBoard(repo);
  const candidates = await buildCandidates(deps, repo, projectId, board);
  if (candidates.length === 0) return;

  await sweepAndReport(ctx, deps, repo, projectId, candidates);
}
