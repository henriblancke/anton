 ...
 * A project with NO remote is the other half of that promise: the push resolves "not-wired" without
 * delivering anything, so the job stays pending on a slow recheck instead of settling `done` on an
 * undelivered write (anton-x7la review) — see SyncNotWiredError.
 *
 ...
 */
import { beads, type SyncOutcome } from "../beads/bd";
...
export interface SyncPushDeps {
  ...
  push?: (repoPath: string) => Promise<SyncOutcome>;
}
...
export function makeSyncPushHandler(deps: SyncPushDeps): JobHandler {
  ...
  return async function syncPush(ctx: JobContext): Promise<void> {
    ...
    const outcome = await push(project.repoPath);

    // "not-wired" resolves rather than rejects, but nothing was published — completing here would
    // mark the job done on a write that never left this machine, with no retry, no park, and no way
    // to resume once a remote is wired. Signal it so the runner keeps the job alive on a slow
    // recheck (no attempt burn, no park storm for a deliberately local-only project) and the very
    // next pass delivers the backlog the moment `anton init` wires a remote.
    if (outcome === "not-wired") {
      throw new SyncNotWiredError(
        `project ${projectId} has no Dolt remote — beads writes stay local until one is wired`,
      );
    }
  };
}
