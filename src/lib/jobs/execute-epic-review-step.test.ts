...
const parseRecordedNarrativeMock = vi.fn<(raw: string | null | undefined) => unknown>();
...
vi.mock("./review-key", () => ({
  ...
  parseRecordedNarrative: (...a: [string | null | undefined]) => parseRecordedNarrativeMock(...a),
}));
...
describe("runReviewStep — resume key", () => {
  it("skips the gate when the resumed row's key matches the current tree", async () => {
    ...
    parseRecordedNarrativeMock.mockReturnValue(recordedNarrative);
    ...
    expect(parseRecordedNarrativeMock).toHaveBeenCalledWith("{...}");
