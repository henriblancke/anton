...
suite("resolveHooksPathOverride (real git)", () => {
  ...
  // The companion gap the test above doesn't cover: an INITIALIZED submodule that's gone stale, not
  // an uninitialized one. `uninitializedSubmoduleSha` only ever returns a value for status `-` (never
  // initialized) — a fast-forward that advances this worktree's OWN branch (and with it, the gitlink)
  // without a following `submodule update --init` leaves an initialized, real, populated checkout
  // whose commit no longer matches what the worktree's own CURRENT tree records. Previously this fell
  // straight through to `return inWorktree`, trusting a real directory that just happens to hold the
  // WRONG commit's hook (PR #263 review, round 28).
  it("does not trust an initialized worktree submodule that has gone stale against the worktree's own current HEAD", async () => {
    const submoduleUpstream = join(sandbox, "hooks-submodule-upstream-worktree-stale");
    mkdirSync(submoduleUpstream);
    execFileSync("git", ["init", "-q", "-b", "main", submoduleUpstream], { stdio: "ignore" });
    execFileSync("git", ["-C", submoduleUpstream, "config", "user.email", "t@example.com"], {
      stdio: "ignore",
    ...
    const worktree = join(sandbox, "worktree-initialized-stale-submodule");
    execFileSync(
      "git",
      ["-C", repo, "worktree", "add", "-q", "-b", "anton/epic-6", worktree, "main"],
      { stdio: "ignore" },
    );
    // Initialize the submodule in the worktree — a real, populated v1 checkout.
    execFileSync(
      "git",
    ...
    // Advance the WORKTREE's own branch to bump the gitlink to v2 — the sort of fast-forward that
    // never re-runs `submodule update` on its own — leaving the checkout at v1 while the worktree's
    // own current HEAD now records v2 for the gitlink.
    ...
    // A real, populated checkout exists — unlike the uninitialized case, `existsSync`/`isDirectory`
    // both pass — but it's the WRONG commit's content. Must not be silently trusted as `inWorktree`.
    expectDisablesHooks(await resolveHooksPathOverride(repo, worktree));
  });

  // The false positive round 28's check itself introduced (PR #263 review, round 36): a checkout
  // disagreeing with `HEAD`'s gitlink is not ALWAYS stale — a task can deliberately bump the
  // submodule to a new commit and STAGE that change (`git add <submodule-path>`) in preparation for a
  // commit that hasn't happened yet. `resolveHooksPathOverride` runs BEFORE that commit
  // (`commitAndPushFix` needs the hooksPath for the commit's own `pre-commit` hook), so `HEAD` still
  // shows the OLD gitlink while the checkout — and the INDEX — already agree on the new one. This must
  // resolve to the real (new) checkout, not disable hooks.
  it("trusts an initialized worktree submodule that disagrees with HEAD but matches a staged, not-yet-committed gitlink bump", async () => {
    const submoduleUpstream = join(sandbox, "hooks-submodule-upstream-worktree-staged-bump");
    mkdirSync(submoduleUpstream);
    execFileSync("git", ["init", "-q", "-b", "main", submoduleUpstream], { stdio: "ignore" });
    execFileSync("git", ["-C", submoduleUpstream, "config", "user.email", "t@example.com"], {
      stdio: "ignore",
    ...
    const worktree = join(sandbox, "worktree-staged-bump-submodule");
    execFileSync(
      "git",
      ["-C", repo, "worktree", "add", "-q", "-b", "anton/epic-staged-bump", worktree, "main"],
      { stdio: "ignore" },
    );
    execFileSync(
      "git",
    ...
    // Deliberately bump the submodule to v2 and STAGE it — but do not commit. `HEAD:hooks` still
    // records v1; the index and the checkout already agree on v2.
    execFileSync("git", ["-C", worktree, "-C", "hooks", "checkout", "-q", v2Sha], { stdio: "ignore" });
    execFileSync("git", ["-C", worktree, "add", "hooks"], { stdio: "ignore" });
    expect(
      execFileSync("git", ["-C", worktree, "rev-parse", "HEAD:hooks"], { encoding: "utf8" }).trim(),
  ...
  // status` then reports no match (the gitlink is gone), so neither the uninitialized nor the
  // initialized-and-current nor the nested-containing-gitlink checks find anything, and execution
  // used to fall straight through to `return inWorktree`, handing back the orphan as if it were an
  // ordinary, trustworthy hooks directory.
  it("disables hooks for an orphaned submodule checkout left behind after its gitlink was deleted", async () => {
    const submoduleUpstream = join(sandbox, "hooks-submodule-upstream-orphaned");
    mkdirSync(submoduleUpstream);
    execFileSync("git", ["init", "-q", "-b", "main", submoduleUpstream], { stdio: "ignore" });
    execFileSync("git", ["-C", submoduleUpstream, "config", "user.email", "t@example.com"], {
      stdio: "ignore",
    ...
    const worktree = join(sandbox, "worktree-orphaned-submodule");
    execFileSync(
      "git",
      ["-C", repo, "worktree", "add", "-q", "-b", "anton/epic-orphan", worktree, "main"],
      { stdio: "ignore" },
    );
    execFileSync(
      "git",
    ...
    expect(readFileSync(join(worktree, "hooks", "pre-push"), "utf8")).toBe("v1\n");
    // A real submodule checkout has its own `.git` gitfile pointing at the superproject's
    // `.git/modules/<name>` (gitrepository-layout(5)).
    expect(existsSync(join(worktree, "hooks", ".git"))).toBe(true);
    const gitFileContents = readFileSync(join(worktree, "hooks", ".git"), "utf8");

    // Simulate the sync merge landing a ref that deletes the submodule: remove the gitlink from the
    // tree, then manually leave the orphaned, `.git`-containing directory behind on disk — mirroring
    // what real git does (it does NOT clean up a deleted submodule's working directory).
    ...
    // `git rm --cached` leaves the working directory in place; if it didn't, recreate it explicitly
    // so the orphan shape (nonempty directory + its own `.git` gitfile, no gitlink left anywhere) is
    // reproduced regardless of git version behavior.
  ...
  // The nested-containing-gitlink counterpart to the staged-bump test above (PR #263 review, round
  // 36): `core.hooksPath=deps/hooks` names a path nested inside the `deps` gitlink, which a task is
  // deliberately bumping and staging (not committing) in the SAME worktree. `containing` — the
  // `HEAD`-tree answer for `deps` — still shows v1; the checkout and the index already agree on v2.
  it("trusts a nested hooksPath's containing submodule when it disagrees with HEAD but matches a staged, not-yet-committed bump", async () => {
    const submoduleUpstream = join(sandbox, "nested-hooks-submodule-upstream-staged-bump");
    mkdirSync(submoduleUpstream);
    execFileSync("git", ["init", "-q", "-b", "main", submoduleUpstream], { stdio: "ignore" });
    execFileSync("git", ["-C", submoduleUpstream, "config", "user.email", "t@example.com"], {
      stdio: "ignore",
    ...
    const worktree = join(sandbox, "worktree-nested-staged-bump-submodule");
    execFileSync(
      "git",
      ["-C", repo, "worktree", "add", "-q", "-b", "anton/epic-nested-staged-bump", worktree, "main"],
      { stdio: "ignore" },
    );
    execFileSync(
      "git",
    ...
    // Deliberately bump `deps` to v2 and STAGE it — but do not commit. `HEAD:deps` still records v1;
    // the index and the checkout already agree on v2.
    execFileSync("git", ["-C", worktree, "-C", "deps", "checkout", "-q", v2Sha], { stdio: "ignore" });
    execFileSync("git", ["-C", worktree, "add", "deps"], { stdio: "ignore" });
    expect(
      execFileSync("git", ["-C", worktree, "rev-parse", "HEAD:deps"], { encoding: "utf8" }).trim(),
    ...
    expect(
      execFileSync("git", ["-C", worktree, "ls-files", "-s", "--", "deps"], { encoding: "utf8" }),
    ).toContain(v2Sha);
    expect(readFileSync(join(worktree, "deps", "hooks", "pre-push"), "utf8")).toBe("v2\n");

    // The checkout disagrees with `HEAD` but matches the staged index — an intentional pending
    // update, not staleness. Must resolve to the real worktree checkout, not disable hooks.
    expect(await resolveHooksPathOverride(repo, worktree)).toBe(join(worktree, "deps", "hooks"));
  });
...
suite("needsHooksPathOverrideForMerge (real git)", () => {
  ...
  // The current checkout matching its OWN gitlink is not enough on its own: `ref` may have moved the
  // gitlink to a NEWER commit than the one currently checked out — a fast-forward changes the
  // recorded gitlink but never re-runs `submodule update` to bring the actual checkout along, so the
  // worktree's submodule content silently goes stale the instant the merge lands, even though this
  // probe would otherwise read "initialized and matching, no override needed" right up until then
  // (PR #263 review, round 19).
  it("returns true when the current submodule checkout is a commit behind the ref's gitlink", async () => {
    const submoduleUpstream = join(sandbox, "hooks-submodule-upstream-behind");
    mkdirSync(submoduleUpstream);
    execFileSync("git", ["init", "-q", "-b", "main", submoduleUpstream], { stdio: "ignore" });
    execFileSync("git", ["-C", submoduleUpstream, "config", "user.email", "t@example.com"], {
      stdio: "ignore",
    ...
    // `feature` bumps the gitlink to v2 and is pushed — the review worktree below never sees this.
    execFileSync("git", ["-C", repo, "checkout", "-q", "feature"], { stdio: "ignore" });
    execFileSync("git", ["-C", repo, "-C", "hooks", "checkout", "-q", v2Sha], { stdio: "ignore" });
    execFileSync("git", ["-C", repo, "add", "hooks"], { stdio: "ignore" });
    execFileSync("git", ["-C", repo, "commit", "-q", "-m", "bump hooks submodule to v2"], {
    ...
    const behindWorktree = join(sandbox, "worktree-behind");
    execFileSync(
      "git",
      ["-C", repo, "worktree", "add", "-q", "-b", "anton/epic-7", behindWorktree, "main"],
      { stdio: "ignore" },
    );
    execFileSync(
      "git",
      ["-C", behindWorktree, "-c", "protocol.file.allow=always", "submodule", "update", "--init", "hooks"],
      { stdio: "ignore" },
    );
    expect(readFileSync(join(behindWorktree, "hooks", "post-merge"), "utf8")).toBe("v1\n");
    execFileSync("git", ["-C", behindWorktree, "fetch", "-q", "origin", "feature"], { stdio: "ignore" });
  ...
  // (PR #263 review, round 21). `resolveHooksPathOverrideForMerge` must return the disabling
  // sentinel here, not `undefined`: `core.hooksPath` IS configured (to `hooks`, the stale v1
  // submodule), so omitting the `-c` override would leave that real, on-disk v1 directory active for
  // the merge — reproduced with git 2.43, where the v1 `post-merge` still fires (PR #263 review,
  // round 26, finding 1's headline case).
  it("resolveHooksPathOverrideForMerge disables hooks rather than reusing a stale value when neither side has the incoming commit", async () => {
    ...
    const v1HookPath = join(submoduleUpstream, "post-merge");
    const v1HookContent = '#!/usr/bin/env sh\ntouch "$(git rev-parse --show-toplevel)/post-merge-ran"\n';
    writeFileSync(v1HookPath, v1HookContent);
    chmodSync(v1HookPath, 0o755);
    execFileSync("git", ["-C", submoduleUpstream, "add", "-A"], { stdio: "ignore" });
    execFileSync("git", ["-C", submoduleUpstream, "commit", "-q", "-m", "v1"], { stdio: "ignore" });
    const v1Sha = execFileSync("git", ["-C", submoduleUpstream, "rev-parse", "HEAD"], {
      encoding: "utf8",
    ...
    expect(readFileSync(join(mergeValueWorktree, "hooks", "post-merge"), "utf8")).toBe(
      v1HookContent,
    );
    ...
    expectDisablesHooks(
      await resolveHooksPathOverrideForMerge(repo, mergeValueWorktree, "origin/feature"),
    );
  });
  ...
  // The nested-hooksPath counterpart to the "disables hooks rather than reusing a stale value" test
  ...
  it("resolveHooksPathOverrideForMerge validates a nested hooksPath against the incoming ref rather than delegating to the current-tree resolver", async () => {
    ...
    expectDisablesHooks(
      await resolveHooksPathOverrideForMerge(repo, nestedMergeWorktree, "origin/feature"),
    );
  });
  ...
  it("resolveHooksPathOverrideForMerge validates a worktree-scoped submodule against the incoming ref rather than returning it unconditionally", async () => {
    ...
    expectDisablesHooks(
      await resolveHooksPathOverrideForMerge(scopedRepo, scopedWorktree, "origin/feature"),
    );
