...
describe("ensureBeadsGitignore (anton init)", () => {
  ...
  // PR #307 review, P1. Adding `formulas/*.bak` is what made these paths reachable with something
  // to write: a repo carrying the four older entries used to take the no-op return and never touch
  // the file, so a linked `.gitignore` was never followed. With a fifth entry it writes — and
  // `writeFileSync` follows a symlink out of the repository entirely.
  describe("refuses to write through a link", () => {
    /** The four pre-`formulas/*.bak` entries: enough that ONLY the new one is missing. */
    const legacyEntries = "issues.jsonl\ninteractions.jsonl\ndolt/\nembeddeddolt/\n";

    it("refuses a symlinked .gitignore and leaves the link's target untouched", async () => {
      dir = await tempDir("anton-gi-");
      const beadsDir = join(dir, ".beads");
      mkdirSync(beadsDir, { recursive: true });
      const outside = join(dir, "outside.txt");
      writeFileSync(outside, legacyEntries);
      symlinkSync(outside, join(beadsDir, ".gitignore"));

      const r = ensureBeadsGitignore(beadsDir);
      expect(r.added).toEqual([]);
      expect(r.refused).toMatch(/SYMLINK/);
      // The escape this closes: the write landing outside the repository.
      expect(await readFile(outside, "utf8")).toBe(legacyEntries);
      // And the link is still a link — refusing must not quietly replace it either.
      expect(lstatSync(join(beadsDir, ".gitignore")).isSymbolicLink()).toBe(true);
    });

    it("refuses when .beads itself is a symlink", async () => {
      dir = await tempDir("anton-gi-");
      const real = join(dir, "elsewhere");
      mkdirSync(real, { recursive: true });
      writeFileSync(join(real, ".gitignore"), legacyEntries);
      const beadsDir = join(dir, ".beads");
      symlinkSync(real, beadsDir);

      const r = ensureBeadsGitignore(beadsDir);
      expect(r.added).toEqual([]);
      expect(r.refused).toMatch(/SYMLINK/);
      expect(await readFile(join(real, ".gitignore"), "utf8")).toBe(legacyEntries);
    });

    // A hard link is an ordinary regular file to `lstat`, so no check can refuse it. The write is
    // safe anyway because it renames a new file into place instead of truncating the shared inode.
    it("updates a hard-linked .gitignore without writing through to its peer", async () => {
      dir = await tempDir("anton-gi-");
      const beadsDir = join(dir, ".beads");
      mkdirSync(beadsDir, { recursive: true });
      const peer = join(dir, "peer.txt");
      writeFileSync(peer, legacyEntries);
      linkSync(peer, join(beadsDir, ".gitignore"));

      const r = ensureBeadsGitignore(beadsDir);
      expect(r.refused).toBeUndefined();
      expect(r.added).toEqual(["formulas/*.bak"]);
      // Only the name inside `.beads/` took the new bytes.
      expect(await readFile(join(beadsDir, ".gitignore"), "utf8")).toContain("formulas/*.bak");
      expect(await readFile(peer, "utf8")).toBe(legacyEntries);
    });

    it("still creates the file when it is simply absent", async () => {
      dir = await tempDir("anton-gi-");
      const beadsDir = join(dir, ".beads");
      mkdirSync(beadsDir, { recursive: true });

      // Absent is the ordinary create case, NOT a refusal — the `missingIsSafe` reading.
      const r = ensureBeadsGitignore(beadsDir);
      expect(r.refused).toBeUndefined();
      expect(r.added).toEqual([...BEADS_GITIGNORE_ENTRIES]);
    });
